...
│   ├── committee_leadership.json # Subcommittee chairs/ranking + tier assignments for all 72 members
│   └── comparison_committees.json # House committee rosters for cross-committee PAC comparison (4 committees, 228 members)
├── scripts/                   # Python pipeline (00–13, standalone scripts)
...
│   ├── 12_committee_comparison.py # Cross-committee PAC comparison (W&M vs 4 other House committees)
│   └── 12_leadership_analysis.py # Leadership vs rank-and-file tier comparison + sector alignment
...
│   │   ├── CommitteeComparisonChart.tsx # Cross-committee PAC comparison horizontal bar chart
...
│   ├── lib/
│   │   ├── data.ts            # Server-side data loaders (16 exports)
...
│   └── data/                  # Static JSON consumed by webapp (13 files)
...
│       ├── committee_comparison.json # Cross-committee PAC comparison (6 entries: W&M + 4 committees + all incumbents)
│       └── leadership_analysis.json # Leadership tier comparison + sector alignment (generated by script 12)
...
12. Cross-committee PAC comparison — compare W&M median PAC receipts vs Armed Services, Appropriations, Energy & Commerce, Financial Services, and all House incumbents (`scripts/12_committee_comparison.py`, standalone)
12. Leadership vs. rank-and-file analysis — tier comparison (Full Committee / Subcommittee / Rank-and-File), per-PAC targeting, subcommittee-sector alignment (`scripts/12_leadership_analysis.py`, standalone, writes directly to `webapp/data/leadership_analysis.json`)
...
- Converts `output/committee_comparison.csv` → `webapp/data/committee_comparison.json`
...
| `config/committee_leadership.json` | Subcommittee chairs/ranking members + tier assignments (Tier 1: full committee leadership, Tier 2: subcommittee leadership, Tier 3: rank-and-file) |
| `scripts/12_leadership_analysis.py` | Standalone analysis: tier comparison, per-PAC targeting, subcommittee-sector alignment. Writes directly to `webapp/data/leadership_analysis.json` |
| `webapp/components/LeadershipChart.tsx` | Client component: tier comparison bar chart + sector alignment table |
...
| `config/comparison_committees.json` | House committee rosters for cross-committee PAC comparison (Armed Services, Appropriations, Energy & Commerce, Financial Services) |
| `scripts/11_classify_employers.py` | 3-tier employer classification: curated map → keyword fallbacks → occupation fallback |
| `scripts/12_committee_comparison.py` | Cross-committee PAC comparison: parses webl24.txt, matches rosters, computes median/mean PAC per committee |
| `webapp/components/CommitteeComparisonChart.tsx` | Client component: horizontal bar chart comparing median PAC receipts across House committees |
| `utils/fec_api.py` | Rate-limited FEC API client (1s delay, 5 retries) |
| `utils/checkpoint.py` | Pipeline checkpoint/resume system |
| `webapp/lib/data.ts` | Server-side data loaders: getMembers, getMemberBySlug, getPacSpread, getBenchmarks, getBeforeAfter, getNews, getSectorColors, getIndustryInfluence, getCommitteeComparison, etc. (16 exports) |
...
Server component. Geographic breakdown charts, employer analysis, PAC contributions for individual member. Displays leadership badges for Tier 1 (coral, "Committee Chair/Ranking Member") and Tier 2 (amber, "Subcommittee Chair/Ranking Member") members; Tier 3 rank-and-file members show no badge. Uses `getLeadershipAnalysis()` to look up member roles.
...
- **"Do Tax-Writers Get More PAC Money?"** — cross-committee PAC comparison (CommitteeComparisonChart): horizontal bar chart showing median PAC receipts for W&M vs Armed Services, Appropriations, Energy & Commerce, Financial Services, and all House incumbents. W&M at $1.2M is 32% above next closest. Data from `committee_comparison.json`
...
- **"Where Power Sits, Money Follows"** — leadership vs. rank-and-file analysis (LeadershipChart client component): horizontal bar chart comparing House W&M tiers (Full Committee Leadership +158%, Subcommittee Leadership +18%, Rank-and-File baseline), subcommittee-sector alignment table showing which chairs get disproportionate PAC money from their jurisdiction's industries (e.g., Social Security chair gets +27pp from Finance & Insurance vs committee average). Data from `leadership_analysis.json`
